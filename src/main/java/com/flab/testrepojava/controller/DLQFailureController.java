...
public class DLQFailureController {
    ...
    // 수동확인을 위한 실패 메시지 조회
    @GetMapping("/failures")
    public ResponseEntity<List<DeadLetterFailureLog>> getFailures() {
        List<DeadLetterFailureLog> failures = dlqFailureService.getAllFailures();
        return ResponseEntity.ok(failures);
    }

    @PostMapping("/failures/{id}/retry")
    public ResponseEntity<String> retrySingleFailure(@PathVariable("id") Long id) {
        String result = dlqFailureService.retryMessageById(id);
        return ResponseEntity.ok(result);
    }
